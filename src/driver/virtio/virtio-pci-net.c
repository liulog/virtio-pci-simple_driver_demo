...
int virtio_pci_net_init(void)
{
    ...
    // enable msix
    virtio_pci_set_config_msix(&gs_virtio_net_hw, 0);        // 对应 MSI-X vector 0
    //virtio_pci_disable_queue_msix(&gs_virtio_net_hw, qnum);
    //virtio_pci_disable_queue_msix(&gs_virtio_net_hw, qnum + 1);
    virtio_pci_set_queue_msix(&gs_virtio_net_hw, qnum, 1);  // 对应 MSI-X vector 1
    virtio_pci_set_queue_msix(&gs_virtio_net_hw, qnum + 1, 2);  // 对应 MSI-X vector 2
